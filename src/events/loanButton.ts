// 💡 대출 요청(/대출개설)에 달린 [수락]/[거절] 버튼을 처리한다. RPS(rpsButton.ts)와 같은
...
// 아니라 그냥 참조 번호라 customId에 노출해도 안전하다. "클릭한 사람 = borrower 본인"인지는
// acceptLoan/declineLoan이 DB에서 조회한 진짜 borrowerId와 비교해서 검증한다.
  ...
  try {
    if (action === 'accept') {
      ...
      await interaction.update({
        content: `✅ <@${loan.borrowerId}>님이 대출 #${loan.id} 요청을 수락했습니다. ${netToBorrower.toLocaleString()}P가 지급되었습니다 (상환 기한: ${loan.dueAt!.toLocaleDateString('ko-KR')}). 상환 시 \`/대출상환 ${loan.id}\`을 사용하세요.`,
        components: [],
      });
      return;
    }
    ...
    if (action === 'decline') {
      ...
      await interaction.update({
        content: `❌ <@${loan.borrowerId}>님이 대출 #${loan.id} 요청을 거절했습니다.`,
        components: [],
      });
      return;
    }
