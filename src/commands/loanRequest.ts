...
export const data = new SlashCommandBuilder()
  ...
  .addUserOption((opt) => opt.setName('빌려줄사람').setDescription('포인트를 빌려줄 유저').setRequired(true))
...
export async function execute(interaction: ChatInputCommandInteraction) {
  const lender = interaction.options.getUser('빌려줄사람', true);
  ...
  try {
    const loan = await requestLoan({
      ...
      lenderId: lender.id,
      lenderIsBot: lender.bot,
      principal,
      dueDays: dueDays ?? undefined,
    });

    // 💡 borrower(요청자)에게 별도 알림은 없다 - 이 메시지 자체가 공개 요청이고, lender가 버튼을
    // 누르면 이 메시지가 결과로 수정(edit)된다.
    await interaction.reply({
      content: `<@${lender.id}>님, <@${interaction.user.id}>님이 ${principal.toLocaleString()} 포인트를 상환 기한 ${loan.dueDays}일로 대출 요청했습니다 (개설 수수료 2% 제외 후 지급). 아래 버튼으로 응답해주세요 (${LOAN_REQUEST_EXPIRY_HOURS}시간 내 미응답 시 만료).`,
